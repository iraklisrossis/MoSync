...
class WidgetInfo {
public:
    WidgetInfo(const string &name): mName(name){
...
int main() {
	try {
        ...
        ConstSet *constSet;
        vector<Constant> *widgetConstants;
        vector<Constant> *defaultPropertiesConstants;
        //vector<Constant> *eventConstants;
        //Trying to find some useful lists of constants
        for (size_t i=0; i < maapi.constSets.size(); i++) {
            constSet = &maapi.constSets[i];
            if(constSet->name == "MAW_"){
                //found the widget name constants
                widgetConstants = &constSet->constants;
            }else if(constSet->name == "MAW_WIDGET_"){
                //found the default widget properties
                defaultPropertiesConstants = &constSet->constants;
            }            
        }
        
        vector<WidgetInfo*> widgets;
        string constMAW("MAW_");
        //We create WidgetInfo for each widget constant
        for(size_t i = 0; i < widgetConstants->size(); i++){
            
            Constant *constant = &(widgetConstants->at(i));
            WidgetInfo *widget = new WidgetInfo(constant->value);
            widgets.push_back(widget);
            
            for (size_t j = 0; j < defaultPropertiesConstants->size(); j++) {
                widget->addProperty(defaultPropertiesConstants->at(j).value);
            }
            
            string widgetPropertiesConst = constMAW + constant->name + "_";

            for (size_t j = 0; j < maapi.constSets.size(); j++) {
                
                constSet = &maapi.constSets[j];

                if(constSet->name == widgetPropertiesConst){
                    for(size_t k = 0; k < constSet->constants.size(); k++){
                        widget->addProperty(constSet->constants[k].value);
                    }
                }           
            }
